...
- **ML pipeline platform filter.** RFC PR A added the `platform` column on shared corpus tables (see Codebase conventions). The streaming `pscanner ml train` path (`pscanner.ml.streaming.open_dataset`) does not yet filter by platform; threading `platform: str = "polymarket"` through `open_dataset` / `_SplitIter` / `SplitDataIter` and surfacing a `--platform` CLI flag is a follow-up. Until that lands, training reads every row regardless of platform — fine while the corpus is single-platform, but a gating concern once Kalshi/Manifold data lands in `training_examples`.
