...
def _final_join_to_v2(scratch: duckdb.DuckDBPyConnection, *, platform: str, now_ts: int) -> None:
    ...
    and the feature projection from the FEATURES registry.
    ...
    """
    # Feature projection emitted from the FEATURES registry (#145).
    # The 40-column SELECT list previously hand-written here is now
    # generated by project_sql() — the single source of truth shared
    # with the Python engine's compute_features. Identity columns,
    # label_won, and JOIN/WHERE scaffolding stay hand-written.
    from pscanner.corpus.feature_projection import project_sql  # noqa: PLC0415

    projection = project_sql()
    ...
    scratch.execute(
        ...
        """,  # noqa: S608 — projection is from a static registry; platform/now_ts are bound below
        [platform, now_ts],
    )
