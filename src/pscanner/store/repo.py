...
@dataclass(frozen=True, slots=True)
class WalletTrade:
    """A single trade by a watched wallet.

    The persistent ``wallet_trades`` table backing this dataclass was removed
    in PR 5 of the dead-code cleanup wave. The dataclass is retained as a
    transient carrier for ``MarketScopedTradeCollector`` (in
    :mod:`pscanner.collectors.market_scoped_trades`), which constructs
    instances in-flight to fan out to subscribers. PR 3 of the same cleanup
    wave deletes the market-scoped collector; once that lands, this dataclass
