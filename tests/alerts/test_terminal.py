...
def test_push_routes_to_correct_detector_buffer() -> None:
    ...
    renderer.push(_alert("subgraph_copy", "a"))

    snapshot = renderer._snapshot()
    assert [a.alert_key for a in snapshot["subgraph_copy"]] == ["a"]


def test_push_buffer_bounded_by_max_per_detector() -> None:
    renderer = TerminalRenderer(max_per_detector=20)
    for i in range(25):
        renderer.push(_alert("subgraph_copy", str(i)))

    snapshot = renderer._snapshot()
    assert len(snapshot["subgraph_copy"]) == 20
    # Oldest 5 dropped, retained keys are 5..24.
    assert [a.alert_key for a in snapshot["subgraph_copy"]] == [str(i) for i in range(5, 25)]
...
async def test_pushing_while_run_active_does_not_crash() -> None:
    ...
    snapshot = renderer._snapshot()
    assert len(snapshot["subgraph_copy"]) == 5
